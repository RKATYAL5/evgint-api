...
const HR_EMAIL = process.env.HR_EMAIL || 'HR@evgint.com';
const ADMIN_EMAIL = process.env.CAREERS_ADMIN_EMAIL || 'hr-admin@evgint.com';
...
function buildApplicationNumber(db) {
  ...
  return `EVG-APP-${new Date().getFullYear()}-${String(db.counters.application).padStart(4, '0')}`;
}
...
const BRAND_NAME = 'Evergreen International';
const BRAND_PRIMARY = '#2f8f5b';
const BRAND_DARK = '#1d4d34';
const BRAND_LOGO_URL = 'https://evgint.com/evergreen-logo-full.png';
const BRAND_SITE_URL = 'https://evgint.com';
...
async function notifyHr(application) {
  ...
  return sendEmail({
    to: HR_EMAIL,
    subject: `New Evergreen International application: ${application.roleLabel}`,
    text: `${summary}\n\nMessage:\n${application.message}`,
...
async function notifyApplicant(application) {
  return sendEmail({
    ...
    subject: `Evergreen International application received: ${application.applicationNumber}`,
    ...
    html: buildEmailHtml({
      ...
      bodyHtml: `<p>Thank you for applying to <strong>${application.roleLabel}</strong> at Evergreen International.</p>
...
async function notifyStatusChange(application) {
  return sendEmail({
    to: application.applicantEmail,
    subject: `Evergreen International application update: ${application.applicationNumber}`,
    text: `Your Evergreen International application status is now "${application.status}". Next update: ${application.nextStep}`,
    html: buildEmailHtml({
...
async function sendApplicantCode({ email, code, mode }) {
  return sendEmail({
    to: email,
    subject: `Evergreen International ${mode === 'signup' ? 'account' : 'sign-in'} code`,
    text: `Your Evergreen International verification code is ${code}. It expires in 10 minutes.`,
    html: buildEmailHtml({
...
async function sendTrainingCode({ email, code, mode }) {
  return sendEmail({
    to: email,
    subject: `Evergreen International Training ${mode === 'signup' ? 'account' : 'sign-in'} code`,
    text: `Your Evergreen International Training verification code is ${code}. It expires in 10 minutes.`,
    html: buildEmailHtml({
      heading: mode === 'signup' ? 'Welcome to Evergreen International Training' : 'Confirm it\u2019s you',
...
async function sendContactNotification({ name, email, phone, subject, message }) {
  ...
  const results = await Promise.allSettled([
    ...
    sendEmail({
      to: email,
      subject: 'We received your message - Evergreen International',
      text: `Thanks for reaching out to Evergreen International. We received your message and will get back to you shortly.\n\nYour message:\n${message}`,
      html: buildEmailHtml({
        heading: 'We received your message',
        bodyHtml: `<p>Thanks for reaching out to Evergreen International. A member of our team will get back to you shortly.</p>
...
async function sendHackathonEnrollmentNotification(fields) {
  ...
  const results = await Promise.allSettled([
    ...
    fields.email
      ? sendEmail({
          to: fields.email,
          subject: 'Hackathon enrollment received - Evergreen International',
          text: `Thanks for enrolling in the Evergreen International Monthly Hackathon. We received your enrollment for ${fields.eventDate || 'the upcoming event'}.`,
          html: buildEmailHtml({
            heading: "You're enrolled!",
            bodyHtml: `<p>Thanks for enrolling in the Evergreen International Monthly Hackathon.</p>
...
server.listen(PORT, () => {
  console.log(`Evergreen International careers API running on http://localhost:${PORT}`);
});

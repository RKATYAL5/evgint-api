...
function getSmtpTransporter() {
  ...
  if (!smtpTransporter) {
    smtpTransporter = nodemailer.createTransport({
      ...
      family: 4, // force IPv4 - see dns.setDefaultResultOrder note above
    });
  }
...
const EMAIL_PROVIDER = RESEND_API_KEY && RESEND_FROM ? 'resend' : getSmtpTransporter() ? 'smtp' : 'none';
...
async function notifyStatusChange(application) {
  return sendEmail({
    ...
    html: `<p>Your Evergreen International application status is now <strong>${application.status}</strong>.</p><p>${application.nextStep}</p>`,
  });
}
...
async function sendApplicantCode({ email, code, mode }) {
  return sendEmail({
    ...
    html: `<p>Your Evergreen International verification code is <strong>${code}</strong>.</p><p>It expires in 10 minutes.</p>`,
  });
}
...
async function sendContactNotification({ name, email, phone, subject, message }) {
  ...
  const results = await Promise.allSettled([
    ...
    sendEmail({
      ...
      html: `<p>Thanks for reaching out to Evergreen International. We received your message and will get back to you shortly.</p><p><strong>Your message:</strong></p><p>${message}</p>`,
    }),
  ]);
  ...
  const results = await Promise.allSettled([
    ...
    fields.email
      ? sendEmail({
          ...
          html: `<p>Thanks for enrolling in the Evergreen International Monthly Hackathon.</p><p>We received your enrollment for <strong>${fields.eventDate || 'the upcoming event'}</strong>.</p>`,
        })
